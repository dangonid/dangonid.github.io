var Game = new (function() {
  ...
  function showSizes() {
    onHomeScreen = false;
    showGame();
    $('#boardsize').html('<span>Pilih Ukuran</span>');
  ...
  function undo() {
    if (!undoStack.length) {
      ...
      if (!undone)
        grid.hint.show('Ini Tombol Undo');
      else
        grid.hint.show('Udah Woy');
      return;
    }
    ...
    tile.mark();
    var s = 'Kotak ini di kembalikan ke ';
    if (value == 1) s += 'red.';
    if (value == 2) s += 'blue.';
    if (value == 0) s += 'keadaan yang kosong.'
    grid.hint.show(s);
    undone = true;
    clearTimeout(checkTOH);
    checkTOH = setTimeout(function(){checkForLevelComplete();}, 700);
